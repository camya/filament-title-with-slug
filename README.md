...
Please read their  [full documentation](https://spatie.be/docs/laravel-translatable/v6/introduction) for all details.
...
Note: This example does not use the [Filament's Translatable trait](https://filamentphp.com/docs/2.x/spatie-laravel-translatable-plugin/getting-started).

**Prerequisite:** Install and configure the [Spatie Laravel Translatable](#spatie-laravel-translatable) composer package.

1. Create an array with a `Tabs\Tab` component for each language.
2. Inside your resource, assign it to the `Tabs` component.

Read the [Filament Tabs Documentation](https://filamentphp.com/docs/2.x/forms/layout#tabs) for more insights.

```php
// 1.
function tabComponentsForTranslation(): array
{
    $locales = ['en' => 'English', 'fr' => 'Français'];
    $tabs = [];
    foreach ($locales as $locale => $localeName) {
        $tabs[] = \Filament\Forms\Components\Tabs\Tab::make($localeName)
            ->schema([
                \Camya\Filament\Forms\Components\TitleWithSlugInput::make(
...
class PostResource extends Resource
{
    public static function form(\Filament\Resources\Form $form): Form
    {
        return $form
            ->schema([
                \Filament\Forms\Components\Tabs::make('Title')
                    ->schema(tabComponentsForTranslation()),
            ]);
    }
}
...
If you get an error like `TypeError htmlspecialchars(): Argument #1 ($string) must be of type string, array given`, you forgot to install and configure the Laravel Translatable package, see Prerequisites.
